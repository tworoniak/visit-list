...
const VisitItem = ({ visit }: { visit: Visit }) => {
  const getBorderClass = () => {
    switch (visit.category) {
      ...
      case "D":
        return "border-indigo-500 border-2 hover:bg-indigo-200";
      default:
        return "";
    }
  };
  ...
  return (
    <div
      className={`rounded-lg bg-gray-300 p-4 w-full flex flex-col items-start hover:bg-gray-200 transition cursor-pointer border-1 border-gray-600 shadow-sm hover:shadow-md transform duration-300 hover:scale-[1.02] ${getBorderClass()}`}
    >
